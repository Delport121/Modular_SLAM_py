...
def ptcloud2sc(ptcloud, sc_shape, max_length):
    ...
    ptcloud = np.asarray(ptcloud.points) # for open3d
    
    num_points = ptcloud.shape[0]
    # num_points = np.asarray(ptcloud.points).shape[0]
...
class ScanContextManager:
    ...
    def __init__(self, shape=[20,60], num_candidates=10, max_radius=8, threshold=0.15, recent_node_exclusion=30): 
        self.shape = shape
        self.num_candidates = num_candidates
        self.threshold = threshold
        self.recent_node_exclusion = recent_node_exclusion # number of recent nodes to exclude in loop detection

        self.max_radius = max_radius # recommended but other (e.g., 100m) is also ok.
    ...
    def addNode(self, node_idx, ptcloud):
        sc = ptcloud2sc(ptcloud, self.shape, self.max_radius)
        rk = sc2rk(sc)
    ...
    def detectLoop(self):        
        exclude_recent_nodes = self.recent_node_exclusion
